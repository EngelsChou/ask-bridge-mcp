...
這是一個單一、完整且可離線執行的安裝包；使用者電腦在安裝或執行時都不需要呼叫 `npm install` 或 `npx` 下載套件。內附的 `npx.cmd` 僅允許以 npm 的 `-y`／`--yes` 參數啟動固定版本 `chrome-devtools-mcp@1.5.0`，不會變成通用套件下載器。日後要升級內附元件時，安裝新版 `ask-bridge-mcp install.exe` 即可。
...
推送與 `package.json` 版本相同的 tag（例如 `v0.2.9`）時，`.github/workflows/release.yml` 會在 GitHub 的 Windows runner 重新建置，並將兩個 EXE 與兩個 SHA-256 檔上傳為 GitHub Release assets。二進位檔不會寫入 Git commit 歷史。
