...
test("rejects ask-bridge older than 0.3.10 before querying with upgrade guidance", async () => {
  const kinds = [];
  const runner = async (invocation) => {
  ...
  await assert.rejects(
    ...
    (error) =>
      /requires ask-bridge 0\.3\.10 or later/i.test(error.message) &&
...
test("caches a successful version check for repeated calls using the same runner", async () => {
  ...
  const runner = async (invocation) => {
    if (invocation.kind === "version") {
      versionChecks += 1;
      return { stdout: "ask-bridge v0.3.10\n", stderr: "" };
    }
    queries += 1;
